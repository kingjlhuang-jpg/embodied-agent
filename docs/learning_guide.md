...
S — 状态空间: 机器人所有可能的状态
    ...
    random-pose physical: 再加入方块sin/cos(yaw) = 50维
...
在 Demo 2 中，策略就是一个神经网络:
  ...
  random-pose physical 输入50维、输出8维（额外观测方块偏航角）
```
...
`sin(yaw), cos(yaw)` 两个输入，位置与角度按 0.25/0.50/1.0 课程训练。
1000 回合全范围结果为 453/1000（45.3%），固定约束激活次数为 0。这个模型
单独保存，不覆盖固定姿态下 85.8% 的物理抓取模型。
