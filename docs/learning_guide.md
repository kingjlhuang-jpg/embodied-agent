...
S — 状态空间: 机器人所有可能的状态
    ...
    physical grasp: grasp状态 + 接触阶段残差状态 = 48维
    random-pose physical: 再加入夹爪—方块相对yaw误差 = 50维
...
在 Demo 2 中，策略就是一个神经网络:
  ...
  physical grasp 输入48维、输出8维（只在靠近后启用残差输入）
  random-pose physical 输入50维、输出8维（额外观测相对偏航误差）
```
...
鲁棒抓取改为 40/8，物理抓取改为 48/8，随机姿态物理抓取改为 50/8，
隐藏层扩大为 256）：
...
方块具有质量、惯性和碰撞体。基线与鲁棒模式会在双指真实碰撞后建立有限力
抓取约束，以抑制轻量级仿真的数值滑移；`--physical` 模式完全禁用该约束，
夹爪重新张开或失去持续接触都会让方块自然滑落。
...
辅助模式会在双指接触后建立有限力固定关节，适合快速验证任务流程，但边缘
接触也可能被锁住。`--physical` 模式完全不调用 `createConstraint`：
...
固定约束             0 N / 从不创建
抓取确认             双指持续接触3步
成功条件             靠摩擦抬高3.5 cm并持续8步
```

夹紧力不是越大越好。过大的位置控制力会把方块挤偏或弹出；有限法向力和
摩擦共同决定最大承重，偏心夹持还会产生使方块旋转的力矩。测试中正中 IK
夹持为 58/60，而沿夹爪轴偏移 4 cm 后为 0/20，说明边缘方块会真实滑落。

物理策略从 24 维基线无损扩展：新增 24 维只在进入 7.5 cm 接触区域后激活，
原网络被冻结，TD3+BC 只能学习接触阶段残差。DAgger 只保留 RL 失败回合，
防止大量容易样本覆盖夹偏和滑落状态。最终纯 RL 盲测为 858/1000（85.8%），
固定约束激活次数为 0。本轮三个在线候选都没有超过 75 回合保护集，最终模型
因此保留无损扩展后的基线权重；这是检查点保护生效，不代表在线更新优于基线。

随机姿态物理任务进一步把位置扩大到 `x=0.31～0.49 m`、
`y=-0.11～0.11 m`，并在完整 `-180°～180°` 内旋转方块。策略从回合开始
观测具有方块 90° 对称性的夹爪—方块相对偏航误差；教师选择最近的等价夹持
方向，远端使用最多 20° 的可达性前倾，并在角度误差小于 8° 前保持夹爪张开。
专项 TD3+BC 训练关闭会拒绝正确转腕标签的 Q-filter，并减弱固定姿态动作锚定。
1000 回合 RL-only 全范围结果从 459/1000（45.9%）提升到
507/1000（50.7%），固定约束激活次数为 0。修复后 10 回合可视化样本为
6/10；主要失败区域仍是 `x>0.43 m` 的机械臂远端。这个模型单独保存，不覆盖
固定姿态下 85.8% 的物理抓取模型。
