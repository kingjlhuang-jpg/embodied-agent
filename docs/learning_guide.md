...
`y=-0.11～0.11 m`，并在 `-45°～45°` 内旋转方块。策略增加
具有方块 90° 对称性的夹爪—方块相对偏航误差两个输入，位置与角度按
0.25/0.50/0.75/1.0 连续课程训练；中间阶段不再因尚未超过全范围检查点而
回滚。1000 回合全范围结果为 459/1000（45.9%），固定约束激活次数为 0。
10 回合可视化样本为 5/10；主要失败区域是 `x>0.43 m` 的机械臂远端。这个
模型单独保存，不覆盖固定姿态下 85.8% 的物理抓取模型。
